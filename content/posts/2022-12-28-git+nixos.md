...
pipeline, or infrastructure as code, but with limited setup and resources
-- a poor mans IAC.

For the setup we will use
* [NixOS](https://nixos.org), a linux distribution that supports a declarative
  style of configuration. This configuration language is how the whole thing is
  set up.
* [Gitolite](https://gitolite.com) allows you to setup git hosting with
  fine-grained access control over git repositories. It also allows us to
  install server-side hooks which we'll use to update the NixOS hosts'
  configuration.
* Docker, a container platform that will allow us to host our services on our
  host.
